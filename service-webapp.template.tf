...
locals {
  ...
  {{service_name}}_dggr_website_domain = "{{website_domain}}.{{environment_config.dggr_hostname}}"
}
...
resource "aws_cloudfront_distribution" "{{service_name}}_website_cdn_root" {
  ...
  origin {
    ...
    domain_name = aws_s3_bucket.{{service_name}}_website_root.bucket_regional_domain_name
